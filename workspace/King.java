...
//All functionality associated with checks is religated to the class Board. (You'll
...
//left
if (col > 0) {
if (!board[row][col-1].isOccupied() || board[row][col -
1].getOccupyingPiece().getColor() != this.getColor()) {
...
//up
if (row > 0) {
if (!board[row-1][col].isOccupied() || board[row-1]
[col].getOccupyingPiece().getColor() != this.getColor()) {
...
if (!board[row][col+1].isOccupied() || board[row]
[col+1].getOccupyingPiece().getColor() != this.getColor()) {
legalMoves.add(board[row][col+1]);
}
//right up
if (row > 0) {
